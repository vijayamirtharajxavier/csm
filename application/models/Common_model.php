...
foreach ($div_id as $key => $row) {
  ...
  public function insertNLedger(){
    ...
    if($ldgalink!="0") {
      $data = array('account_name' => strtoupper($ldgname),'account_type'=>$ldgaccttype, 'acclink_id'=>$ldgalink,'import_account'=>$importaccode,'op_balance'=>$opbalance);     
...
//$mmyy = date("mY", strtotime($mm_yy));
        ...
        $editdbaccName=$this->input->post('editdbaccountname');
        $editcraccName=$this->input->post('editcraccountname');
  //      $debitaccNumber = $this->input->post('editdebitaccountNumber');
   //     $creditaccNumber = $this->input->post('editcreditaccountNumber');
        ...
        $spdbaccname = explode("-",$editdbaccName);
        $debitaccNumber=trim($spdbaccname[0]);

     
        $spcraccname = explode("-",$editcraccName);
        $creditaccNumber=trim($spcraccname[0]);   
...
$finyear=$this->session->userdata('finyear');
         $this->db->where(array('id'=>$ejid,'fyear' => $finyear));
...
public function delTD($fyear,$flag)
{


        $where_trns= array('fyear'=>$fyear,'shrtd_flag'=>$flag);
        $this->db->where($where_trns);
        $status = $this->db->delete('soc_dividend_tdinterest_tbl');    
...
public function getOpShareTrans($fyear,$mem_id)
{
  $sql="SELECT * FROM soc_openingbalance_tbl where fyear=? and acclink_id=?";
  $query = $this->db->query($sql,array($fyear,$mem_id)); 
  return $query->result_array();

}

public function getShareTrans($st_date,$en_date,$shr_acc,$mem_id)
{
  $sql="SELECT trans_date, (CASE WHEN db_cr<>'PJ' THEN trans_amount ELSE 0 END)`rcpt`,(CASE WHEN db_cr='PJ' THEN trans_amount ELSE 0 END)`pymt` FROM `soc_trans_tbl` WHERE (account_id=? and cr_account_id=? or account_id=? and cr_account_id=?) AND delflag=0 AND trans_date>=? and trans_date<=?";

  //$sql="SELECT * FROM soc_trans_tbl where trans_date>=? and trans_date<=? and (account_id=? and cr_account_id=? or cr_account_id=? and account_id=?) and delflag=0";
  $query = $this->db->query($sql,array($shr_acc,$mem_id,$shr_acc,$mem_id,$st_date,$en_date)); 
return $query->result_array();
}
...
//        $sql = "SELECT r.id, r.receipt_number,r.receipt_date,r.account_id,r.account_name,r.receipt_amount,r.narration,l.account_name as 
  //      `cash_bank` FROM soc_receipt_tbl r, soc_ledgermaster_tbl l where r.cash_bank=l.id and r.receipt_date>=? and r.receipt_date<=? and r.delflag=0 ORDER by r.receipt_date desc";
    ...
    public function getallothPayment($fdt=null,$tdt=null,$finyear=null)
    {
     
        $sql = "SELECT r.* FROM soc_payment_2021_tbl r where dm_flag=1 and r.trans_date>=? and r.trans_date<=? and  r.fyear=?";
        $query = $this->db->query($sql, array($fdt,$tdt,$finyear));
        return $query->result_array();
    }


    public function getallacPayment($finyear=null)
    {
     
        $sql = "SELECT * FROM soc_accountclosed_tbl  where  fyear=? order by trans_date desc";
   ...
   $sql = "SELECT * FROM soc_payment_2021_tbl  where (trans_type='CNTR' or trans_type='PYMT') and trans_date>=? and trans_date<=? and  fyear=? ORDER by trans_date";
  ...
  //      $query = $this->db->query($sql, array($mmyy));
   ...
   return ($status === true ? true : false);   
      ...
      public function deleteac($id)
      {
       ...
       // $this->db->where('trans_refid',$id);         
        //$status = $this->db->update('soc_payment_2021_tbl', $upd_trns);         
        $status = $this->db->delete('soc_accountclosed_tbl');         
   ...
   return ($status === true ? true : false);   
      ...
      public function deletepay($id)
      {
       ...
       // $this->db->where('trans_refid',$id);         
        //$status = $this->db->update('soc_payment_2021_tbl', $upd_trns);         
        $status = $this->db->delete('soc_payment_2021_tbl');         
...
$upd_trnsdata = array(
'trans_date'=> $editpayDate,
'debit_account' => $editcash_bank,
'trans_amount' => $editpayAmount,
'credit_account'=> $editaccNumber,
'trans_refid' => $edittransrefid,
'trans_narration' => $editpayNarration);

$this->db->where('id',$id);
$status= $this->db->update('soc_payment_2021_tbl', $upd_trnsdata);
return ($status === true ? true : false);   
}
...
public function deletedemand($id)
 {
   ...
   return ($status === true ? true : false);   
      ...
      public function dmd_accounts()
      {
        $sql="SELECT * FROM `soc_ledgermaster_tbl` WHERE dmd_flag=1 ORDER by account_type DESC";
        $query = $this->db->query($sql);
        return $query->result_array();
      }

      public function dmd_data($sql_qry)
      {
        $sql=$sql_qry;
        $query = $this->db->query($sql);
        return $query->result_array();
...
public function ins_acpayment($data)
{
  //$pymt_array =array("debit_account"=>$debit_id,"credit_account"=>$credit_id,"db_cr"=>"DB","trans_amount"=>$cheque_amount,"trans_date"=>$ac_date,"trans_id"=>$journal_id,"trans_refid"=>$cheque_ref,"trans_type"=>"PYMT","cheque_ref"=>$cheque_date,"trans_narration"=>$narration, "fyear"=>$finyear,"dm_flag"=>0);
$this->db->insert('soc_accountclosed_tbl',$data);
  $insert_id = $this->db->insert_id();
return $insert_id;
}


public function ins_payment($data)
{
  $this->db->insert('soc_payment_2021_tbl',$data);
  $insert_id = $this->db->insert_id();
...
{
  $sql="SELECT * FROM soc_eventcategory_tbl where delflag=0";
  $query = $this->db->query($sql);
  return $query->result_array();
}

    public function get_allnotificationbyid($id=null)
    {
      $sql="SELECT e.id,e.event_date,e.from_date,e.to_date,e.event_name,e.delflag,e.category_id,c.category_name FROM soc_events_tbl e,soc_eventcategory_tbl c where e.category_id=c.id and e.id=?";
      $query = $this->db->query($sql,array($id));
      return $query->result_array();     
...
{
  $sql="SELECT e.id,e.event_date,e.from_date,e.to_date,e.event_name,e.delflag,e.category_id,c.category_name FROM soc_events_tbl e,soc_eventcategory_tbl c where e.category_id=c.id";
  $query = $this->db->query($sql);
  return $query->result_array();     
...
public function get_noticeboard()
{
  ...
  $delflag=0;
       $sql = "SELECT * FROM soc_events_tbl where to_date>='" . $curr_date . "' and isactive=0 and delflag=" . $delflag;
        $query = $this->db->query($sql);
        return $query->result_array();     
...
public function getShrTd()
{
  $sql="SELECT * FROM soc_ledgermaster_tbl where shrtd_flag=1";
  $query = $this->db->query($sql);
  return $query->result_array();

}

public function getShrTdData($td_acc,$preyear)
{
  $sql="SELECT * FROM soc_dividend_tdinterest_tbl where shrtd_flag=? and fyear=?";
  $query = $this->db->query($sql,array($td_acc,$preyear));
  return $query->result_array();

}


public function getShrTdsumData($td_acc,$preyear)
{
  $sql="SELECT member_id,sum(nod)`tnod`,sum(trans_amt)`trans_amt`,int_pc,sum(int_amount)`int_amount` FROM `soc_dividend_tdinterest_tbl` WHERE shrtd_flag=? and fyear=? GROUP BY member_id";
//  $sql="SELECT member_id, FROM soc_dividend_tdinterest_tbl where shrtd_flag=? and fyear=?";
  $query = $this->db->query($sql,array($td_acc,$preyear));
  return $query->result_array();

}

public function getShrTDSummary()
{
  $sql="SELECT d.shrtd_flag,d.int_pc,sum(d.trans_amt)`trans_amt`,sum(d.int_amount) `int_amt`, d.curr_timestamp FROM `soc_dividend_tdinterest_tbl` d GROUP BY shrtd_flag";
  $query = $this->db->query($sql);
  return $query->result_array();

}

public function ins_shrtdint($data_ins)
{
  $this->db->insert_batch("soc_dividend_tdinterest_tbl", $data_ins);
//return ($status === true ? true : false);   
        if($this->db->affected_rows() > 0)
        {
          $status = array("success"=>true,"messages"=>"Process completed successfully!!!"); 
        }
        else
        {
         $status = array("success"=>false,"messages"=>"Process Failed!!!");  
        }
...
//$ofdata = $this->input->post('duetoamount');
       ...
       for($i = 0; $i < count($this->input->post('duetoaccid')); $i++) {

$ins_subonote[] = $this->input->post('duetoaccid')[$i];
$sdata[]= $this->input->post('duetoamount')[$i];  
 
}
/*
for ($row=0; $row < count($ins_subonote); $row++) { 
  // code...
 // if($data[$row] !=0.00)
 // {
$ins_subdata[] = array("onote_id"=>$offnoteNum,"offnote_date"=>$offnoteDate,"fyear"=>$finyear, "member_id"=>$b_memberid,"acclink_id"=>$ins_subonote[$row],"amount"=>$sdata[$row]);
//}
...
*/
        ...
        for($rw = 0; $rw < count($this->input->post('duetoamount')); $rw++) {
//            $data[] = array("acclink_id"=>$this->input->post('duetoaccid')[$rw],"amount"=> $this->input->post('duetoamount')[$rw]);
  
$data[]= $this->input->post('duetoamount')[$rw];  


//$data_id[]=$this->input->post('duetoaccid')[$rw];
...
$drs_dep = $data[4];
$oth_amt = $data[5];
$tot_due = $mt_lout+$mt_lint+$mshr_cap+$fxd_dep+$drs_dep+$oth_amt;
...
for ($row=0; $row < count($ins_subonote); $row++) { 
 ...
 // if($data[$row] !=0.00)
 // {
$ins_subdata[] = array("onote_id"=>$offnoteNum,"offnote_date"=>$offnoteDate,"fyear"=>$finyear, "member_id"=>$b_memberid,"acclink_id"=>$ins_subonote[$row],"amount"=>$sdata[$row],"record_link_id"=>$offnote_tbl_id);
//}
...
$this->db->insert_batch('soc_subofficenote_tbl',$ins_subdata);
 ...
 //$this->output->enable_profiler(TRUE);
    ...
    public function updateofficenote()
    {
      $finyear=$this->session->userdata('finyear');    
        //Application Data
        date_default_timezone_set("Asia/Calcutta"); 
        $recid = $this->input->post('edit_id');
        $offnoteNum = $this->input->post('edit_officenote_id');
        $offnoteDate = $this->input->post('edit_officenote_date');
        $loanappid = $this->input->post('edit_loanappn_id');
        $b_memberid = $this->input->post('mem_id');
      //  $b_membername = $this->input->post('bmember_name');
        $s_memberid = $this->input->post('smem_id');
        $cash_bank=$this->input->post('edit_cash_bank');
        //$s_membername = $this->input->post('smember_name');
        $res_id = $this->input->post('edit_resolution_number');
        $res_date = $this->input->post('edit_resolution_date');
        $chq_number = $this->input->post('edit_cheque_number');
        $chq_date = $this->input->post('edit_cheque_date');
        $chq_name = $this->input->post('edit_cheque_name');
        $r_words = $this->input->post('rupees_words');
        $sts = $this->input->post('edit_ofstatus');
...
//$ofdata = $this->input->post('duetoamount');
       ...
       for($i = 0; $i < count($this->input->post('edit_duetoaccid')); $i++) {

$ins_subonote[] = $this->input->post('edit_duetoaccid')[$i];
$sdata[]= $this->input->post('edit_duetoamount')[$i];  
 
}

        for($rw = 0; $rw < count($this->input->post('edit_duetoamount')); $rw++) {
//            $data[] = array("acclink_id"=>$this->input->post('duetoaccid')[$rw],"amount"=> $this->input->post('duetoamount')[$rw]);
  
$data[]= $this->input->post('edit_duetoamount')[$rw];  
         }       
...
//$ins_subonote=
  ...
  $upd_officenoteTbl = array('officenote_id' => $offnoteNum,
    'onote_date' => $offnoteDate,
  'loan_appno'  => $loanappid,
  'member_id' => $b_memberid,
  'bank_cash'=>$cash_bank,
...
$update_memberTbl = array('surety_id'=>$s_memberid,'share_capital'=>$mshr_cap,
  ...
  */          
  ...
  $status= $this->db->update('soc_officenote_trans_tbl', $upd_officenoteTbl);
...
for ($row=0; $row < count($ins_subonote); $row++) { 
 ...
 // if($data[$row] !=0.00)
 // {
$ins_subdata[] = array("onote_id"=>$offnoteNum,"offnote_date"=>$offnoteDate,"fyear"=>$finyear, "member_id"=>$b_memberid,"acclink_id"=>$ins_subonote[$row],"amount"=>$sdata[$row],"record_link_id"=>$recid);
//}

}

      
$this->db->where('record_link_id',$recid);
$this->db->delete('soc_subofficenote_tbl');
...
$finyear=$this->session->userdata('finyear');
 $insert_transactionTbl=array("db_cr"=> "DB","trans_type"=> $trans_type,"cheque_ref"=>$chq_ref . "/" . $bank_detail ,"credit_account"=> $accNumber,"debit_account"=>$cash_bank,"trans_amount"=>$payAmount,"div_id"=>$divid, "trans_id"=>$payNum,"trans_date"=>$payDate,"trans_refid"=>$transrefid, "trans_narration"=> $payNarration,"fyear"=>$finyear);

            
       $status= $this->db->insert('soc_payment_2021_tbl', $insert_transactionTbl);
...
public function insertupdateEventNotification()
{
  $addupd_flg= $this->input->post('addupdflag');
  $rec_id = $this->input->post('recid');
  $finyear=$this->session->userdata('finyear');
  $fm_date= $this->input->post('fmDate');
  $to_date = $this->input->post('toDate');
  $event_name=$this->input->post('notification_content');
  $cat_id = $this->input->post('category_id');

$ins_upd_events=array(
  "event_date"=>$fm_date,
  "from_date"=>$fm_date,
  "to_date"=>$to_date,
  "category_id"=>$cat_id,
...
if($addupd_flg=="0")
{      
 $status= $this->db->insert('soc_events_tbl', $ins_upd_events);
...
else
{
  ...
  $status= $this->db->update('soc_events_tbl', $ins_upd_events);
} 
...
$memData = $this->getMembers_dataById($mNumber);
if($memData)
{
  foreach ($memData as $key => $mvalue) {
...
//print_r($insert_loanappnTbl);
      ...
      {
        $this->db->where("id",$rec_id);
        $status= $this->db->update('soc_loanapplication_tbl', $insert_loanappnTbl);
      } 
...
{
  $this->db->where('id',$id);
  $status=$this->db->delete('soc_events_tbl');
  return ($status === true ? true : false);  
}
...
public function fetchofficenote($fyear=null)
{
$sql="select * from soc_officenote_trans_tbl where  fyear=?";
$query = $this->db->query($sql,array($fyear));
//$this->output->enable_profiler(TRUE);
return $query->result_array();
}
...
public function fetchloanapplnbyid($fyear=null,$id=null)
{
$sql="select * from soc_loanapplication_tbl where  fyear=? and id=?";
$query = $this->db->query($sql,array($fyear,$id));
//$this->output->enable_profiler(TRUE);
return $query->result_array();
}

public function fetchloanappln($fyear=null)
{
$sql="select * from soc_loanapplication_tbl where  fyear=?";
$query = $this->db->query($sql,array($fyear));
//$this->output->enable_profiler(TRUE);
return $query->result_array();
}

public function fetchofficenotebyid($fyear=null,$id=null)
{
$sql="select * from soc_officenote_trans_tbl where  fyear=? and id=?";
$query = $this->db->query($sql,array($fyear,$id));
//$this->output->enable_profiler(TRUE);
return $query->result_array();
}

public function getontesub($finyear=null,$link_id=null)
{
  $sql="SELECT t.id,t.acclink_id,t.amount,m.duetoaccount FROM soc_subofficenote_tbl t,soc_officenote_master_tbl m where t.record_link_id=? and t.fyear=? and t.acclink_id=m.acclink_id";
  $query=$this->db->query($sql,array($link_id,$finyear));
  return $query->result_array();
}
...
/*SELECT t.cr_account_id,t.account_id,t.db_cr,SUM(CASE WHEN (t.cr_account_id=3060 AND t.account_id=539) THEN COALESCE(t.trans_amount,'0') END) 'credit',SUM(CASE WHEN (t.account_id=3060 AND t.cr_account_id=539) THEN COALESCE(t.trans_amount,'0') END) 'debit' FROM soc_trans_tbl t WHERE  t.trans_type='JOUR'  and fyear='2019' AND (cr_account_id='3060' or account_id='3060')
*/  
   $sql="SELECT t.cr_account_id,t.account_id,t.db_cr,t.trans_date,t.trans_id,t.trans_type,SUM(CASE WHEN (t.cr_account_id=? AND t.account_id=?) THEN COALESCE(t.trans_amount,'0') END) 'credit',SUM(CASE WHEN (t.account_id=? AND t.cr_account_id=?) THEN COALESCE(t.trans_amount,'0') END) 'debit' FROM soc_trans_tbl t WHERE  t.trans_type='JOUR' and fyear=? and (cr_account_id=? or account_id=?) order by account_id" ;
 }
else
 {
   $sql="SELECT t.cr_account_id,t.account_id,t.db_cr,t.trans_date,t.trans_id,t.trans_type,SUM(CASE WHEN (t.cr_account_id=? AND t.account_id=?) THEN COALESCE(t.trans_amount,'0') END) 'credit',SUM(CASE WHEN (t.account_id=? AND t.cr_account_id=?) THEN COALESCE(t.trans_amount,'0') END) 'debit' FROM soc_trans_tbl t WHERE  t.trans_type='JOUR'  and fyear=? and (cr_account_id=? or account_id=?) order by account_id" ;
  ...
  $query = $this->db->query($sql,array($memid,$acctid,$memid,$acctid,$finyear,$memid,$memid));
 // print_r($query);
  return $query->result_array();
  }
...
function getAcJvData($id)
{
    $finyear=$this->session->userdata('finyear');
    
  $sql="SELECT * FROM soc_journal_2021_tbl where ac_id=? and fyear=?";
  $query=$this->db->query($sql,array($id,$finyear));
  return $query->result_array();

}

    function get_acpaymentbyid($id)
    {
      $finyear=$this->session->userdata('finyear');
       $delflag=0;
        $sql = "SELECT * FROM soc_accountclosed_tbl where id=? and fyear=?";
        $query = $this->db->query($sql,array($id,$finyear));
       //print_r($query);
        return $query->result_array();
    ...
    function get_paymentbyid($id)
    {
      $finyear=$this->session->userdata('finyear');
       $delflag=0;
        $sql = "SELECT * FROM soc_payment_2021_tbl where id=? and fyear=?";
        $query = $this->db->query($sql,array($id,$finyear));
       //print_r($query);
        return $query->result_array();
...
function get_journalbyid($id)
   ...
   function get_settings_byid($finyear) {
...
function get_loandetails($lnid=null)
{
  ...
  $sql="SELECT * FROM soc_loanapplication_tbl where app_number=? and fyear=?";
  $query = $this->db->query($sql,array($lnid,$finyear));
  return $query->result_array();
}
...
function insDemand($dmdData=null)
{
  $this->db->insert_batch('soc_demand_2021_tbl',$dmdData);
}


function insertExcel($jvdata=null)
{
  $this->db->insert_batch('soc_journalentry_tbl',$jvdata);
}

public function delOffnote($id=null)
{
  $this->db->where('id', $id);
  $status = $this->db->delete("soc_officenote_trans_tbl");
  return ($status === true ? true : false);   
